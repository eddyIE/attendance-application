...
@section('content')
    ...
    {{-- Form chọn lớp --}}
    <form action="index/course" method="POST">
        @csrf
    ...
    <div id="studentRecords">
        <form action="{{ route('create') }}" method="POST">
            ...
            @isset($currentCourse)
                <input type="hidden" name='current-course-id' value='{{ $currentCourse->id }}'>
            @endisset
                        ...
                        <tr>
                            ...
                            <td class="text-center border border-0">
                                <input type="radio" class="btn-check" name="students[{{ $loop->index + 1 }}][status]"
                                    value="" id="{{ $each->id }}_status" checked>
                                <label class="btn btn-outline-success" for="{{ $each->id }}_status">
                                    Có mặt
                                </label>
                            </td>
                            <td class="text-center border border-0">
                                <input type="radio" class="btn-check" name="students[{{ $loop->index + 1 }}][status]"
                                    value="without reason" id="{{ $each->id }}no_reason">
                            ...
                            <td class="text-center border border-0">
                                <input type="radio" class="btn-check" name="students[{{ $loop->index + 1 }}][status]"
                                    id="{{ $each->id }}with_reason" autocomplete="off" value="with reason">
            ...
            @endphp
            Ngày điểm danh: <input type="date" name='current-date' class='pt-2 pb-2 ps-2 mb-2 me-4 text-primary fs-5'
                value="{{ $date }}>
            Giờ bắt đầu:
            <input type="time" class="me-4 pt-2 pb-2 ps-2 mb-2 timepicker" id="start" name="start" min="06:00" max="23:00"
                required>
            Giờ kết thúc:
